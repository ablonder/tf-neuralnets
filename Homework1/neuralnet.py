...
class NeuralNet():
    ...
    def buildnet(self, layers):
        ...
        for l in range(1, len(layers)):
            ...
            self.net.append(tf.Variable(np.random.uniform(-.1, .1, size = (layers[l-1], layers[l])), name = "w" + str(l)))
            # create the biases for this layer
            self.net.append(tf.Variable(np.random.uniform(-.1, .1, size = (1, layers[l])), name = "b" + str(l)))
            # calculate the output from this layer
            self.net.append(tf.sigmoid(tf.matmul(self.net[-3], self.net[-2]) + self.net[-1], name = "o" + str(l)))
        ...
        self.error = tf.reduce_mean(tf.square(self.targets-self.net[-1]), name = "error")
    ...
    def test(self, data = None):
        ...
        return self.sess.run([self.error], feed_dict = feeder)[0]
